...
APP_VERSION = "0.1.1-shadow-orchestrator"
...
def call_worker(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    r = requests.post(f"{STRATEGY_WORKER_URL.rstrip('/')}/evaluate", headers={"Authorization": f"Bearer {WORKER_TOKEN}", "Content-Type": "application/json"}, json=payload, timeout=20)
    if not r.ok:
...
def ingest_candidate(candidate: CandidateIn, credentials: HTTPAuthorizationCredentials | None = Security(bearer_scheme)):
    ...
    write_event(c["id"], "INGESTED", payload={"source": "structured_ingestion", "version": APP_VERSION})
    worker = call_worker(candidate_to_worker_payload(c, data_timestamp=None, trigger_confirmed=False, triggered_at=None))
