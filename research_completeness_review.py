...
def prepare_checklist(packet, request, draft, reference):
    ...
    prefix = ['research'] if request['prompt_version'] in ('governed-research-v16','governed-research-v17') else []
    coverage = draft_value(draft, prefix + ['materiality_coverage'])
    scope = draft['assessment_scope'] if request['prompt_version']=='governed-research-v17' else coverage['assessment_scope']
...
def record_completeness_review(packet, request, draft, reference, assessment, now):
    ...
    coverage = draft_value(draft, (['research'] if request['prompt_version'] in ('governed-research-v16','governed-research-v17') else []) + ['materiality_coverage'])
    scope = draft['assessment_scope'] if request['prompt_version']=='governed-research-v17' else coverage['assessment_scope']
    ...
    supplied_scope = assessment['scope'] if request['prompt_version']=='governed-research-v17' else review.scope
