...
APP_VERSION = '0.1.0-shadow-execution'
...
def consume(c:HTTPAuthorizationCredentials|None=Security(bearer)):
    ...
    for s in rows:
        ...
        if not EXECUTOR_TOKEN:
            ...
            continue
        r=requests.post(f"{EXECUTOR_URL.rstrip('/')}/paper/bracket", headers={'Authorization':f'Bearer {EXECUTOR_TOKEN}','Content-Type':'application/json'}, json=payload, timeout=20)
        if not r.ok:
...
def monitor(c:HTTPAuthorizationCredentials|None=Security(bearer)):
    ...
    for a in active:
        ...
        if not EXECUTOR_TOKEN:
            ...
            continue
        r=requests.get(f"{EXECUTOR_URL.rstrip('/')}/paper/order/{oid}",headers={'Authorization':f'Bearer {EXECUTOR_TOKEN}'},timeout=20)
        if not r.ok:
